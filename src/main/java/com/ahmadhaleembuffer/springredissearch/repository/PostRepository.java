package com.ahmadhaleembuffer.springredissearch.repository;
...
import com.ahmadhaleembuffer.springredissearch.model.Post;
...
public class PostRepository {
    ...
    public Page search(String content, Set<String> tags, Integer page) {
        ...
        List<Post> postList = searchResult.getDocuments()
                .stream().map(this::convertDocumentToPost)
                .collect(Collectors.toList());

        return Page.builder().posts(postList)
                .total(totalResults)
    ...
    public List<CategoryStats> getCategoryWiseTotalPost() {
        ...
        aggregationBuilder.groupBy("@tags", Reducers.count().as("NO_OF_POST"), Reducers.avg("@views").as("AVERAGE_VIEWS"));
        ...
        LongStream.range(0, aggregationResult.totalResults).mapToObj(idx -> aggregationResult.getRow((int) idx))
                .forEach(row -> {
                    categoryStatsList.add(CategoryStats.builder().totalPosts(row.getLong("NO_OF_POST"))
